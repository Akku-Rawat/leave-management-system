...
type Errors = { leaveType?: string; fromDate?: string; toDate?: string; reason?: string; };
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm p-6">
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          <div>
            ...
            <select value={leaveType} onChange={(e) => setLeaveType(e.target.value)} className={field(errors.leaveType)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="date" min={today} value={fromDate} onChange={(e)=>setFromDate(e.target.value)} className={field(errors.fromDate)} />
              {errors.fromDate && <p className="text-xs text-red-600 mt-1">{errors.fromDate}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-2">To</label>
              <input type="date" min={fromDate || today} value={toDate} onChange={(e)=>setToDate(e.target.value)} className={field(errors.toDate)} />
              {errors.toDate && <p className="text-xs text-red-600 mt-1">{errors.toDate}</p>}
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-600 mb-2">Reason</label>
            <textarea rows={4} value={reason} onChange={(e)=>setReason(e.target.value)} className={field(errors.reason)} placeholder="Briefly explain…" />
