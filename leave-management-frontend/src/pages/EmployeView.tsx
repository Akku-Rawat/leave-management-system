            ...
            <div className="lg:w-3/5 p-8 bg-slate-50">
              <div className="max-w-2xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 bg-white rounded-2xl shadow-md p-6">
                  <div>
                    <label htmlFor="type" className="block text-sm font-semibold">Leave Type</label>
                    <select id="type" value={formData.type} onChange={handleChange} required className="w-full border rounded-xl p-3 bg-slate-50">
                  ...
                  <div>
                    ...
                    <select id="duration" value={formData.duration} onChange={handleChange} className="w-full border rounded-xl p-3 bg-slate-50">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <input type="date" id="startDate" value={formData.startDate} onChange={handleChange} required className="w-full border rounded-xl p-3 bg-slate-50" />
                    <input type="date" id="endDate" value={formData.endDate} onChange={handleChange} required className="w-full border rounded-xl p-3 bg-slate-50" />
                  </div>
                  <textarea id="reason" value={formData.reason} onChange={handleChange} required rows={4} placeholder="Reason..." className="w-full border rounded-xl p-3 bg-slate-50" />
                  <div className="flex gap-4">
                    <button
                      type="button"
                      onClick={() =>
                        setFormData({ type: "", duration: "full", startDate: "", endDate: "", reason: "", emergencyContact: "" })
                      }
